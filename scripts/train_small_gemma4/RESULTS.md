# Gemma4-12B DSpark Small-Data Results

This file records the first successful small-data end-to-end reproduction run on branch `dev/train_small_data`.
...
- This is a smoke-test baseline, not an expected paper-level result.
...
- Confidence calibration looks reasonable in aggregate (low ECE), but the base acceptance rate is very low, so these metrics should not be over-interpreted.
